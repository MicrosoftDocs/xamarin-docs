...
> [!NOTE]
> The width values for columns are set as `*`` by default in Xamarin.Forms, which ensures that the column will fill the available space.` The Height values for Rows are set as `*`` as well by default in Xamarin.Forms. 

...
> [!WARNING]
> By default, access keys can be activated when a modal dialog is displayed, for example by the `DisplayAlert` and `DisplayPromptAsync` methods. However, custom logic can be written to disable access keys in this scenario. This can be achieved by handling the `Dispatcher.AcceleratorKeyActivated` event in the `MainPage` class of your UWP project, and in the event handler setting the `Handled` property of the event arguments to `true` when a modal dialog is displayed.

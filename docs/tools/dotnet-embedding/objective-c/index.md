...
When a managed type overrides `Equals(Object)` or `GetHashCode` then it generally means the defaut (.NET) behaviour was not the best one. We can assume the default Objective-C behaviour would not be either.
...
When a single managed type extends several types then multiple Objective-C categories are generated.
...
To expose the right representation of the C# API to Objective-C, we add `NS_UNAVAILABLE` to any initializer that is not present in the child class from the parent class.

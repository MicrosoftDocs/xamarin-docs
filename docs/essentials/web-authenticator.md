...
[Microsoft Authentication Library (MSAL)](https://docs.microsoft.com/en-us/azure/active-directory/develop/msal-overview) provides an excellent turn-key solution to adding authentication to your app. There's even support for Xamarin apps in their client NuGet package.
...
In your `MainActivity` you will also need to add a call to Essentials in your `OnResume`:
...
To add Apple Sign In to your apps, first you'll need to [configure your app to use Apple Sign In](https://docs.microsoft.com/en-us/xamarin/ios/platform/ios13/sign-in).
...
It's possible to use the `WebAuthenticator` API with any web backend, but using ASP.NET Core requires minimal effort.

First you need to setup all your desired providers just like you would setup any [external social authentication provider](https://docs.microsoft.com/en-us/aspnet/core/security/authentication/social/?view=aspnetcore-3.1&tabs=visual-studio) in an ASP.NET Core web app.

There are two additional things needed for this example to work:

1. Set the Default Authentication Scheme to `CookieAuthenticationDefaults.AuthenticationScheme` in your `.AddAuthentication()` call
2. Use `.AddCookies()` in your Startup.cs `.AddAuthentication()` call.
3. All providers must be configured with `.SaveTokens = true;`
...
Sometimes you may want to return things like the provider's `access_token` back to the app which you can do via the callback URI's query parameters. Or, you may want to instead create your own identity on your server and pass back your own token to the app. What and how you do this part is up to you!

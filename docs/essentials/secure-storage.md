...
2. Create a new XML file named `auto_backup_rules.xml` in `Resources/xml`. Then set the following content that includes all shared preferences except for `SecureStorage`.

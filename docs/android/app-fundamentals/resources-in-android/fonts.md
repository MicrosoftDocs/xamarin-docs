...
To create a a font family, first add all the fonts to the **Resources/font** folder. Then create a new XML file in the font folder for the font family. This XML file will have a root `font-family` element that contains one or more `font` elements. Each `font` element declares the attributes of a font. 

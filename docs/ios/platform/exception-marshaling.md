...
# Exception Marshaling in Xamarin.iOS and Xamarin.Mac
...
libraries (the Mono runtime or CoreCLR and the Objective-C runtime libraries)
...
For legacy Xamarin projects (pre-.NET), the default action is to do nothing.
For the sample above, this means letting the Objective-C runtime unwind
managed frames. This action is problematic, because the Objective-C runtime
doesn't know how to unwind managed frames; for example, it won't execute any
`catch` or `finally` clauses in that frame.
...
When the above code sample _is_ effective, it's because Objective-C has a method of being notified of unhandled
...
4. Xamarin.iOS/Xamarin.Mac's handler will convert the Objective-C exception
   ...
   as where the Objective-C exception was thrown.
...
This scenario is commonly found in Xamarin.iOS apps, because on the main
...
Catching exceptions on the managed-native boundary isn't cost-free, so for
legacy Xamarin projects (pre-.NET), it's not always enabled by default:
...
The [Build-time flags](#build_time_flags) section explains how to enable
interception when it's not enabled by default (or disable interception when it
is the default).
...
- `Default`: The default varies by platform. It's always
  `ThrowObjectiveCException` in .NET. For legacy Xamarin projects, it's
  ...
  in the future.
- `UnwindNativeCode`: This is the previous (undefined) behavior. This isn't
  available when using the GC in cooperative mode (which is the only option on
  watchOS; thus, this isn't a valid option on watchOS), nor when using
  CoreCLR, but it's the default option for all other platforms in legacy
  Xamarin projects.
- `ThrowObjectiveCException`: Convert the managed exception into an
  ...
  default in .NET and on watchOS in legacy Xamarin projects.
...
- `Default`: The default varies by platform. It's always
  `ThrowManagedException` in .NET. For legacy Xamarin projects, it's
  ...
  in the future.
- `UnwindManagedCode`: This is the previous (undefined) behavior. This isn't
  available when using the GC in cooperative mode (which is the only valid GC
  mode on watchOS; thus this isn't a valid option on watchOS), nor when using
  CoreCLR, but it's the default for all other platforms in legacy Xamarin projects.
- `ThrowManagedException`: Convert the Objective-C exception to a managed
  exception and throw the managed exception. This is the default in .NET and
  on watchOS in legacy Xamarin projects.
- `Abort`: Abort the process.
- `Disable`: Disables the exception interception, so it doesn't make sense to
  set this value in the event handler, but once the event is raised, it's too
  late to disable it. In any case if set, it will abort the process.
